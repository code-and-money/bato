...
type BatoProps<T> = T extends (props: infer U) => string ? Omit<U extends undefined ? never : U, keyof ClassProp> : never;

type Bato = <_ extends "iternal use only", V>(
  config: V extends BatoVariantShape
    ? BatoConfigBase & {
        variants?: V;
...
// compose
type Compose = <T extends ReturnType<Bato>[]>(
  ...components: [...T]
) => (props?: (UnionToIntersection<{ [K in keyof T]: BatoProps<T[K]> }[number]> | undefined) & ClassProp) => string;
...
// assemble
function assemble(options?: BatoConfigOptions): { compose: Compose; cx: (...classes: ClassValue[]) => string; bato: Bato } {
        ...
        if (props[key] in (incompatible as Incompatible)[key]!) {
          ...
          for (const incompatible of Object.keys(incompatibles)) {
            if (incompatible in props && incompatibles[incompatible]?.includes(props[incompatible])) {
              // deno-fmt-ignore
              throw new Error(
                `You called bato with incompatible variatns: { ${key}: "${props[key]}"} incompatible with {  ${incompatible}: "${props[incompatible]}"}`,
              );
            }
          }
        }
